...
task board polling) → **`v0.9.0`** (`start_task` + task status sync + provenance badge +
per-environment snippet alias).
...
- **Agent workflow follow-ups** (v0.9.0): `start_task` MCP tool (self-claim on an existing task a
