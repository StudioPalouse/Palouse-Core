...
import { backfillAuditChain } from './audit-backfill.js';
...
try {
  ...
  // Chain any historical audit rows before the new chaining code serves traffic.
  const chained = await backfillAuditChain(db as unknown as Database);
  console.log(
    `Audit hash chain: ${chained.rowsChained} rows chained across ${chained.workspaces} workspaces`,
  );
} catch (err) {
