...
export const auditEvents = pgTable(
  'audit_events',
  {
    ...
    at: ts('at'),
    // Tamper-evident hash chain (per-workspace). Nullable so historical rows can
    // be backfilled in order after the columns land; every new write is chained
    // synchronously via appendAuditEvent. See packages/shared/src/audit-chain.ts.
  ...
  (t) => ({
    ...
    workspaceSeqUq: uniqueIndex('audit_events_workspace_seq_uq').on(t.workspaceId, t.seq),
  }),
);
