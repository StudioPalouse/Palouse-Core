...
function DangerZoneCard({ workspace }: { workspace: Workspace }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setStatus({
        kind: 'ok',
        text: 'Check your email for a link to finish deleting this account. It expires in 1 hour.',
      });
      setName('');
      setOpen(false);
    } catch (err) {
  ...
  return (
    <Card className="border-destructive/40">
      <CardHeader>
        ...
        <CardDescription>
          Deleting the account permanently removes this workspace and everything in it: tasks,
          agents, integrations, and members. This cannot be undone.
        </CardDescription>
      </CardHeader>
        ...
        {!open ? (
          ...
          <form onSubmit={submit} className="flex flex-col gap-3">
            ...
            <p className="text-muted-foreground text-xs">
              We will email you a confirmation link. The account is only deleted after you click it.
            </p>
...
function SettingsContent() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {workspace?.role === 'owner' && <DangerZoneCard workspace={workspace} />}
    </div>
  );
}
