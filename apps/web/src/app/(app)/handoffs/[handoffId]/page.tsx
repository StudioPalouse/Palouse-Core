...
function durationLabel(detail: HandoffDetail): string | null {
  ...
  const end =
    isTerminal(handoff.state) || handoff.state === 'needs_review'
...
function ActivityReportContent() {
  ...
  return !detail ? (
    ...
    <div className="flex flex-col gap-6">
      ...
      <p className="rounded-lg border p-4 text-sm leading-relaxed">{detail.narrative.headline}</p>
        ...
        <>
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-muted-foreground text-xs">
              Costs are computed by Palouse from its model price catalog at the moment of ingest and
              stay reproducible. Each row stores the exact rates used.
            </p>
          </div>
        </>
      )}
